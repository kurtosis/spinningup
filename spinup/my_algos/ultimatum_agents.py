...
class StaticDistribBot:
    """
    Bot that plays a draw from a static distribution.
    """
    ...
    def act(self):
        offer = (
            1 + np.tanh(self.mean_offer + self.std_offer * np.random.randn(1)[0])
        ) / 2
        threshold = (
            1
            + np.tanh(self.mean_threshold + self.std_threshold * np.random.randn(1)[0])
        ) / 2
        ...
        for t in range(steps_per_epoch):
            ...
            if end_episode:
                if done or episode_capped:
                    logger.store(EpRet1=episode_return[0], EpRet2=episode_return[1], EpLen=episode_length)
                obs = env.reset()
                episode_return = np.array([0.0, 0.0])
                episode_length = 0
...
# def dualultimatum_sac(
#     env_fn,
#     agent_fn=DualUltimatumSACAgent,
#     seed=0,
#     epochs=100,
#     steps_per_epoch=4000,
#     replay_size=1000000,
