...
def vpg_mod(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), env_kwargs=dict(), seed=0,
        steps_per_epoch=4000, epochs=50, gamma=0.99, pi_lr=3e-4,
        vf_lr=1e-3, train_v_iters=80, lam=0.97, max_ep_len=1000,
        logger_kwargs=dict(), save_freq=10, print_freq=1, verbose=False):
    """
    Vanilla Policy Gradient
    ...
    for epoch in range(epochs):
        for t in range(local_steps_per_epoch):
            ...
            ep_ret += r
            ep_len += 1
            # print(f'e {epoch} ; t step {t} ; ep_len {ep_len} ; d {d}')

            # save and log
            buf.store(o, a, r, v, logp)
            logger.store(VVals=v)
            logger.store(Actions=a[0])
            if verbose:
                if epoch == 0 or (epoch + 1) % print_freq == 0:
                    print(f'ep_len {ep_len} ; t {t} ; a {np.round(a,4)} ; r {np.round(r,4)} ; ep_ret {np.round(ep_ret,4)} ; v {np.round(v,4)} ; logp {logp}')
        ...
        # Log info about epoch
        logger.log_tabular('Epoch', epoch)
        logger.log_tabular('Actions', with_min_and_max=True)
        logger.log_tabular('EpRet', with_min_and_max=True)
        logger.log_tabular('EpLen', average_only=True)
        logger.log_tabular('VVals', with_min_and_max=True)
        logger.log_tabular('TotalEnvInteracts', (epoch + 1) * steps_per_epoch)
        ...
        # print(f'epoch {epoch}')
        if epoch==0 or (epoch+1) % print_freq == 0:
            # for p in ac.pi.beta_net.parameters():
            #     print(p)
            logger.dump_tabular()
        else:
            logger.flush()
